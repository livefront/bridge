...
/**
 * A wrapper class for a {@link Bitmap} that can be placed into a {@link android.os.Bundle} that may
 * be written to disk.
 */
class BitmapWrapper implements Parcelable {
  ...
  protected BitmapWrapper(Parcel in) {
    ...
    if (bytes != null) mBitmap = BitmapFactory.decodeByteArray(bytes, 0, bytes.length);
  }
  ...
  public static final Creator<BitmapWrapper> CREATOR =
      new Creator<BitmapWrapper>() {
        @Override
