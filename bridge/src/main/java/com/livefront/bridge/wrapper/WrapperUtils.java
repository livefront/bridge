 ...
 * Bundle} unwrapped, these objects will cause a crash if the {@code Bundle} is written to a {@link
 * android.os.Parcel} that then calls {@link Parcel#marshall()}.
 */
  ...
  public static void unwrapOptimizedObjects(@NonNull Bundle bundle) {
    ...
    for (String key : keys) {
      if (bundle.get(key) instanceof BitmapWrapper) {
        ...
        if (bitmapWrapper != null) bundle.putParcelable(key, bitmapWrapper.getBitmap());
      }
    }
  }
  ...
  public static void wrapOptimizedObjects(@NonNull Bundle bundle) {
    ...
    for (String key : keys) {
      if (bundle.get(key) instanceof Bitmap) {
        ...
        if (bitmap != null) bundle.putParcelable(key, new BitmapWrapper(bitmap));
      }
    }
  }
}
